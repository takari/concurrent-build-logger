 ...
 * 
 * @see http://projects.lidalia.org.uk/sysout-over-slf4j/
 */
public class SLF4JPrintStream extends PrintStream {
  ...
  private final WeakHashMap<Thread, LineSplitter> splitters = new WeakHashMap<>();
  ...
  private LineSplitter splitter() {
    ...
    synchronized (splitters) {
      LineSplitter buffer = splitters.get(thread);
      if (buffer == null) {
        buffer = new LineSplitter();
        splitters.put(thread, buffer);
      }
      return buffer;
    }
  }
  ...
  public PrintStream append(char value) {
    ...
    if (isPrivileged(caller)) {
      ...
      log(caller, String.valueOf(String.valueOf(value)));
    }
    return this;
  }
